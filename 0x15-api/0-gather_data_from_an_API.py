...
with urllib.request.urlopen(f"https://jsonplaceholder.typicode.com/users/{argv[1]}") as response:
    html = response.read()
    html = json.loads(html.decode())
    name = html.get("name")
    print(f"Employee {name} is done with ", end="")

with urllib.request.urlopen(f"https://jsonplaceholder.typicode.com/users/{argv[1]}/todos") as response:
    html = response.read()
    html = json.loads(html.decode())
    done = 0
    task = 0
